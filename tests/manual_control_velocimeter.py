...
class ManualControlViewer:
    ...
    def init_pygame(self, width: int, height: int) -> None:
        ...
        pygame.display.set_caption("RoarPy Manual Control Viewer (Training Observation)")
    ...
    try:
        while True:
            ...
            if now - last_print_time >= 1.0:
                local_vel = np.asarray(obs.get("local_velocimeter"), dtype=np.float32)
                accel = np.asarray(obs.get("accelerometer"), dtype=np.float32)
                raw_local_vel = np.asarray(
                    env.unwrapped.local_velocimeter_sensor.get_last_gym_observation(),
                    dtype=np.float32,
                )
                raw_speed = float(np.linalg.norm(raw_local_vel))
                print(
                    "local_velocimeter(norm): "
                    f"{local_vel} accelerometer(norm): {accel} "
                    f"raw_local_vel_mps: {raw_local_vel} raw_speed_mps: {raw_speed:.2f}"
                )
                last_print_time = now
